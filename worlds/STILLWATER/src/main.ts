...
function animate(): void {
  ...
  world.boat.position.y = world.boat.userData.baseY + bob;
  world.boat.rotation.z = Math.sin(time * 1.1) * 0.04;
  world.boat.rotation.x = Math.sin(time * 0.8) * 0.02;

  if ((world.scene.userData as { mistGroup?: THREE.Group }).mistGroup) {
    const mistGroup = (world.scene.userData as { mistGroup: THREE.Group }).mistGroup;
    mistGroup.position.x = Math.sin(time * 0.11) * 1.2;
    mistGroup.rotation.y = time * 0.03;
  }

  world.moon.rotation.z = time * 0.025;
